...
def get_CP_loss(net_list, target_feature_list, poison_batch, s_coeff_list, tol=1e-6):
    ...
    for net, s_coeff, target_feat, poison_feat_mat in zip(net_list, s_coeff_list, target_feature_list, poison_feat_mat_list):
...
def get_CP_loss_end2end(net_list, target_feature_list, poison_batch, s_coeff_list, tol=1e-6):
    """
    Corresponding to one step of the outer loop (except for updating and clipping) of Algorithm 1
    """
...
def make_convex_polytope_poisons(subs_net_list, target_net, base_tensor_list, target, device, opt_method='adam',
                                 ...
                                 tol=1e-6, start_ite=0, poison_init=None, end2end=False):
    target_net.eval()
    ...
    for ite in range(start_ite, iterations):
        ...
        total_loss, s_init_coeff_list = cp_loss_func(subs_net_list, target_feat_list, poison_batch, s_init_coeff_list, tol=tol)
        ...
        if ite % 50 == 0 or ite == iterations - 1:
            ...
            target_loss, target_init_coeff = cp_loss_func([target_net],
...
def train_network_with_poison(net, target_img, poison_tuple_list, base_idx_list, chk_path, args, save_state=False):
    ...
    if args.retrain_opt == 'adam':
        ...
        optimizer = torch.optim.SGD(params, lr=args.retrain_lr, momentum=args.retrain_momentum, weight_decay=args.retrain_wd)
    ...
    if not args.end2end:
        ...
        poisoned_loader = torch.utils.data.DataLoader(FeatureSet(poisoned_loader, net), batch_size=64, shuffle=True)
